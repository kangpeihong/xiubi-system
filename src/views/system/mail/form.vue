<template>
  ...
  >
    <el-form
      ...
      label-width="80px"
    >
      <el-form-item label="邮寄信息" v-if="isAdd">
        <el-input
          v-model="form.remark"
          style="width: 380px;"
          rows="5"
      ...
      <el-form-item label="邮寄信息" v-else>
        <el-input
          v-model="froms.info"
          style="width: 380px;"
          rows="5"
...
export default {
  ...
  methods: {
    ...
    doSubmit() {
      ...
      if (this.isAdd && this.form.remark !== '') {
        let data = {
          info: this.form.remark
        }
        mail.addMailMessage(data).then(res => {
          this.dialog = false
          this.$parent.getList()
          if (res.info !== '') {
            this.$notify({
        ...
        if (data.info !== '') {
          mail.alterMailMessage(data).then(res => {
